...
public class Tests {
    ...
    public class MachineThread extends Thread {
        ...
        @Override
        public void run() {
            // Modified to capture the returned jobName for UR5 and UR6 validation
    ...
    public void testUR5() {
        System.out.println("\n--- Running UR5 Test (Verify Returned Job Names) ---");
        JobShopManager manager = new JobShopManager("FCFS");

        Job job1 = new Job("SuperSecretJob", List.of(new Operation("FDM", 5), new Operation("SLA", 3)));
        manager.specifyJobs(List.of(job1));

        // When these start, they should print "SuperSecretJob"
        new MachineThread(manager, "FDM", 1).start();
        new MachineThread(manager, "SLA", 1).start();
